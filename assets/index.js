(() => {
  ...
  const feedbackTemplates = window.SK_FEEDBACK_TEMPLATES || {
    universal: {
      label: 'Универсальный',
  ...
  landing.hidden = !isDemo;
  widgetPage.hidden = isDemo;
  ...
  function renderQuickReplies(templateKey) {
    const template = feedbackTemplates[templateKey] || feedbackTemplates.universal || Object.values(feedbackTemplates)[0];
    const grid = $('#quickReplies');
    grid.replaceChildren(...template.replies.map(text => {
  ...
  function configure(shop) {
    ...
    if (!isDemo) {
      ...
      title.id = 'shopName';
      title.textContent = $('#shopName').textContent;
      $('#shopName').replaceWith(title);
      document.title = 'Отзыв: ' + (shop.name || 'Сервис Контроль');
    }
    reset();
  }

  async function loadShop() {
    $('#widgetLoading').hidden = false;
    $('#loadError').hidden = true;
    try {
      if (!validId(id?.trim())) {
        throw new Error('В ссылке указан некорректный номер стойки. Проверьте QR-код или обратитесь к сотруднику заведения.');
      }
      const data = await request('https://api.github.com/gists/' + SK.gistId, {
      ...
      if (!shops || Array.isArray(shops) || !Object.hasOwn(shops, id) || !shops[id] || typeof shops[id] !== 'object') {
        throw new Error('Эта стойка ещё не подключена. Пожалуйста, сообщите сотруднику заведения.');
      }
      configure(shops[id]);
    } catch (error) {
      $('#widgetLoading').hidden = true;
  ...
  card.querySelectorAll('.star-btn').forEach(button => button.addEventListener('click', () => {
    ...
    if (rating < 4) {
      $('#negativeForm').hidden = false;
      return;
    }
    const yandex = safeUrl(config.yandex_url || config.maps_url);
    const gis = safeUrl(config.gis_url);
    if (isDemo || (yandex && gis)) {
      $('#btnYandex').href = yandex;
      $('#btn2Gis').href = gis;
      $('#demoMapsNote').hidden = !isDemo;
      show('chooseMapBlock', true);
    } else if (yandex || gis) {
      const target = yandex || gis;
      $('#redirectLink').href = target;
      show('redirectBlock', true);
      redirectTimer = setTimeout(() => location.assign(target), 1600);
    } else {
      show('noMapsBlock', true);
    }
  }));

  card.querySelectorAll('[data-back]').forEach(button => button.addEventListener('click', () => {
    reset();
    $('.star-btn').focus({ preventScroll: true });
  }));

  $('#quickReplies').addEventListener('click', event => {
    const button = event.target.closest('.tag-choice');
  ...
  card.querySelectorAll('.maps-link').forEach(link => link.addEventListener('click', event => {
    if (isDemo) {
      ...
      $('#demoMapsNote').textContent = 'В рабочей форме здесь откроется страница вашего заведения на картах.';
    }
  }));

  $('#negativeForm').addEventListener('submit', async event => {
    event.preventDefault();
    if (sending || rating < 1 || rating > 3 || !config) return;
    message($('#sendError'), '');
    const button = $('#btnSend');
    if (isDemo) {
      $('#successText').textContent = 'Так выглядит подтверждение отправки. Это демо — ваши данные никуда не отправлены.';
      $('#successReset').hidden = false;
      show('successBlock', true);
      return;
    }
    ...
    card.querySelectorAll('.star-btn,.tag-choice').forEach(element => { element.disabled = true; });
    try {
      const data = await request('/api/send', {
        ...
        body: JSON.stringify({
          shopId: id,
          rating,
          tags: [...card.querySelectorAll('.tag-choice[aria-pressed=true]')].map(element => element.textContent),
      ...
      if (!data.success || !Array.isArray(data.results) || !data.results.some(result => result.success === true)) {
        throw new Error('Не удалось доставить обращение. Текст сохранён в форме — попробуйте ещё раз или обратитесь к сотруднику.');
      }
      $('#successText').textContent = 'Обращение доставлено в настроенный канал руководства. Спасибо, что помогаете улучшить сервис.';
      show('successBlock', true);
    } catch (error) {
  ...
  $('#retryLoad').addEventListener('click', loadShop);
  $('#successReset').addEventListener('click', reset);
